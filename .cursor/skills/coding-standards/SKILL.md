...
description: Review code against this repository's coding standards. Use when reviewing changes, pull requests, or assessing whether code fits the repository.
---

# Coding Standards

Use this during review to judge whether a change fits the repository. Do not duplicate formatter,
linter, typecheck, build, or commit rules that automated checks and agent prompts already enforce.
...
Instead, create a config layer for that function, then instantiate that differently in tests and in production. This helps keep the code cleaner. Don't make this layer optional - that adds more indirection to all layers of the code.
